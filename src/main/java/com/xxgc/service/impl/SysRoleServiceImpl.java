...
import com.xxgc.mapper.SysRoleMapper;
...
public class SysRoleServiceImpl extends ServiceImpl<SysRoleMapper, SysRole>
    ...
    public Map<String, Object> querySubMenuTree(Map<String, Object> rootMap,
                                                List<SysMenu> sysMenus, List<SysMenu> roleMenu) {
        //subMenu
        List<Map<String, Object>> subMenu = new ArrayList<>();
        //选出某一个根节点下面的子树，递归。
        List<SysMenu> sub = sysMenus.stream().filter(item ->
                rootMap.get("value") == (item.getPid())).collect(Collectors.toList());
        //取出pid为0的节点
        if (!sub.isEmpty()) {
            sub.forEach(item -> {
